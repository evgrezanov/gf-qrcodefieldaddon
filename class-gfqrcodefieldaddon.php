...
class GFQRcodeFieldAddOn extends GFAddOn {
	...
	public function pre_init() {
		...
		if ( $this->is_gravityforms_supported() && class_exists( 'GF_Field' ) ) {
			require_once( 'includes/class-qrcode-gf-field.php' );
		}
	}
	...
	public function scripts() {
		$scripts = array(
			array(
				'handle'  => 'html5-qrcode',
				'src'     => $this->get_base_url() . '/asset/html5-qrcode.min.js',
	...
	public function tooltips( $tooltips ) {
		$simple_tooltips = array(
			'input_class_setting' => sprintf( '<h6>%s</h6>%s', esc_html__( 'Input CSS Classes', 'simplefieldaddon' ), esc_html__( 'The CSS Class names to be added to the field input.', 'gf-qrcodefieldaddon' ) ),
		);
